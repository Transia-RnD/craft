...
pub extern "C" fn emit() -> i32 {
    ...
    let output_len = unsafe { function_param(0, STI_UINT256.into(), nft_id.as_mut_ptr(), nft_id.len()) };
    ...
    unsafe {
        ...
        // Add nftid field
        let mock_nft_id: [u8; 32] = [0xAB; 32]; // mock hash256
        if add_nftid_field(txn_index, &mock_nft_id) < 0 {
            return -5;
        }
